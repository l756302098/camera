...
class HK_Api(object):
    ...
    def put_continuous(self,ip,p,t,z,channel=1,username="admin",psd="123qweasd"):
        url = "http://"+ip+"/ISAPI/PTZCtrl/channels/"+str(channel)+"/continuous"
        params = {
            'pan':p,
            'tilt':t,
            'zoom':z
        }
        root = self.Continuous_to_xml(params)
        f = BytesIO()
