...
if __name__ == '__main__':
    try:
        ...
        print(device_ip,device_port,device_username,device_password,xml_path,channel)
        ...
        check = True
        while(check):
            length,ok = api.get_pixelToPixelParam(device_ip,channel,device_username,device_password)
            if ok :
                if length == str(pixel_bytes):
                    print("bytes length ",length)
                    check = False
                else:
                    ok = api.put_pixelToPixelParam(xml_path,device_ip,pixel_bytes,channel,device_username,device_password)
            sleep(1)
        print("end check temp data lenght")
        print("start check video type")
        check = True
        while(check):
            video_type,ok = api.get_streamParam(device_ip,channel,device_username,device_password)
            if ok :
                if video_type == 'pixel-to-pixel_thermometry_data':
                    check = False
                else:
                    ok = api.put_streamParam(device_ip,channel,device_username,device_password)
            sleep(1)
        ...
        while not rospy.is_shutdown():
            ...
            if data != None:
                ...
                data_array = data[20:]
                image_size = image_height * image_width * pixel_bytes
