...
class MqttToStrBridge(Bridge):
    ...
    def _create_ros_message(self, mqtt_msg: mqtt.MQTTMessage) -> rospy.Message:
        """ create ROS message from MQTT payload """
        # Hack to enable both, messagepack and json deserialization.
        payload_str = str(mqtt_msg.payload, encoding = "utf-8")
        ros_str = String()
        ros_str.data = payload_str
        return ros_str
        # ros_msg = self._resolve_json(payload_str)
        # return ros_msg
    
    def _resolve_json(self,json_str: str) -> DiagnosticStatus:
        rospy.logdebug("resolve_json {}".format(json_str))
        rospy.logdebug(type(json_str))
        ...
        try:
            ...
            for k,v in json_dict.items():
                ...
                rospy.logdebug(v)
                if isinstance(v, str):
                    kv_item = KeyValue(key = k, value = v)
                    status.values.append(kv_item)
                else:
                    kv_item = KeyValue(key = k, value = str(v))
                    status.values.append(kv_item)
        except Exception as e:
            rospy.logerr("resolve_json error:")
            rospy.logerr(e)
        return status

class StrToMqttBridge(Bridge):
    """ Bridge from ROS topic to MQTT
    ...
    def _publish(self, msg: rospy.Message):
        e_data = extract_values(msg)
        # rospy.logdebug(type(e_data))
        # rospy.logdebug("extract_values {}".format(e_data))
        if  'data' in e_data:
            data = e_data['data']
            self._mqtt_client.publish(topic=self._topic_to, payload=data)

__all__ = ['create_bridge', 'Bridge', 'RosToMqttBridge', 'MqttToRosBridge', 'MqttToKVBridge', 'StrToMqttBridge']
